...
namespace OpusWrapper
{
    ...
    public class StreamState
    {
        ...
        private readonly uint[] _crcLookup = new UInt32[]
            {
        ...
        private readonly uint _bytesPerSegment;
        private UInt64 _totalBits;
        private Int32 _bos;
        private UInt32 _pageNo;
        private UInt32 _serialNo;

        public bool Eos { get; set; }

        public UInt64 GranulePos { get; set; }
        ...
        public UInt32 SerialNo
        {
            get
            {
                if (_serialNo == 0)
                {
                    ...
                    _serialNo = (UInt32)random.Next(1, int.MaxValue);
                }
                return _serialNo;
            }
            set { _serialNo = value; }
        ...
        {
            int unsavedPaketIndex = 0;
            while (_packets.Count > 0)
            {
                var og = GetPage(ref unsavedPaketIndex);
                og.Write(stream);
            }
            _totalBits = 0;
        }
        ...
        {
            return Math.Max((int)(_totalBits / byte.MaxValue) + 1, _packets.Count) / byte.MaxValue;
        }

        private byte[] _notEncodedBuffer2 = new byte[0];
        ...
        {
            int savedPacketCount;
            int unsavedCount;
            var table = CreateSegmentalTable(unsavedPaketIndex, out savedPacketCount, out unsavedCount);

            var oggsHeader = new OggsHeader(_pageNo, SerialNo, table, _packets[savedPacketCount - 1].GranulePos,
                                            unsavedPaketIndex != 0, _bos == 0, Eos && savedPacketCount == _packets.Count);
            ...
            {
                Header = oggsHeader,
                HeaderLen = oggsHeader.Data.Length,
                BodyLen = oggsHeader.SegmentTableSum,
                Body = new byte[oggsHeader.SegmentTableSum]
            };

            int saved = 0;
            var savingPaket = _packets.FirstOrDefault();
            foreach (var count in table)
            {
                Array.Copy(savingPaket.PacketData, unsavedPaketIndex, og.Body, saved, count);
                saved += count;
                if (count == savingPaket.PacketDataLength - unsavedPaketIndex)
                {
                    _packets.Remove(savingPaket);
                    unsavedPaketIndex = 0;
                    savedPacketCount--;
                    savingPaket = _packets.FirstOrDefault();
                }
                else
                {
                    unsavedPaketIndex += count;
                }
                _totalBits -= count;
            }
            SetPageCheckSum(og);
        ...
        private byte[] CreateSegmentalTable(int unsavedPaketIndex, out int savedPacketCount, out int unsavedPacketIndex)
        {
            var segmentTable = new List<byte>();
            int packet = -1 * unsavedPaketIndex;
            unsavedPacketIndex = 0;
            savedPacketCount = 0;
            for (int index = 0; index < _packets.Count && segmentTable.Count < byte.MaxValue; index++)
            {
                packet += _packets[index].PacketDataLength;
                savedPacketCount++;
                do
                {
                    segmentTable.Add(packet > byte.MaxValue ? byte.MaxValue : (byte)packet);
                    packet -= byte.MaxValue;
                } while (packet > 0 && segmentTable.Count < byte.MaxValue);
                if (packet > 0)
                {
                    unsavedPacketIndex = packet;
                }
                packet = 0;
            }
        ...
        private void SetPageCheckSum(Page og)
        {
            UInt32 crcReg = 0;
            /* safety; needed for API behavior, but not framing code */
            og.Header.CrcChecksum = 0;

            for (int i = 0; i < og.HeaderLen; i++)
            {
                crcReg = (crcReg << 8) ^ _crcLookup[((crcReg >> 24) & 0xff) ^ og.Header.Data[i]];
            }

            for (int i = 0; i < og.BodyLen; i++)
            {
                crcReg = (crcReg << 8) ^ _crcLookup[((crcReg >> 24) & 0xff) ^ og.Body[i]];
            }
